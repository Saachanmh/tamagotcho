...
function drawMonster (
    ...
    // Dessine l'accessoire par défaut (traits) puis les accessoires équipés
    drawAccessory(ctx, traits.accessory, traits.accentColor, bodyY, pixelSize, frame, equippedAccessories)
...
// drawAccessory now accepte l'accessoire des traits et les accessoires équipés
function drawAccessory (
    ...
    // Dessine les accessoires équipés (venant de la boutique)
    Object.values(equippedAccessories).forEach((item) => {
        if (!item || item.visible === false) return
        drawAccessoryItem(ctx, item, bodyY, pixelSize, frame)
    })
}

function drawAccessoryItem (
    ctx: CanvasRenderingContext2D,
    item: AccessoryItem,
    bodyY: number,
    pixelSize: number,
    ...
    switch (item.type) {
        case 'hat':
            ctx.fillStyle = color
            ctx.fillRect(60, bodyY - 20, pixelSize * 6, pixelSize) // bande
            ctx.fillRect(66, bodyY - 28, pixelSize * 4, pixelSize) // sommet
            ctx.fillStyle = '#00000022'
            ctx.fillRect(66, bodyY - 20, pixelSize * 4, Math.max(1, Math.floor(pixelSize / 2)))
            break

        case 'glasses':
            ctx.fillStyle = color
            ctx.fillRect(60, bodyY + 20, pixelSize * 3, pixelSize) // gauche
            ctx.fillRect(96, bodyY + 20, pixelSize * 3, pixelSize) // droite
            ctx.fillRect(69, bodyY + 20, pixelSize * 2, Math.max(1, Math.floor(pixelSize / 2))) // pont
            ctx.globalAlpha = 0.25
            ctx.fillStyle = '#FFFFFF'
            ctx.fillRect(61, bodyY + 21, pixelSize * 2, pixelSize - 2)
            ctx.fillRect(97, bodyY + 21, pixelSize * 2, pixelSize - 2)
            ctx.globalAlpha = 1
            break
        ...
        case 'shoes':
            ...
            ctx.fillRect(105, bodyY + 54, pixelSize * 3, pixelSize * 2)
            ctx.fillStyle = '#00000022'
            ctx.fillRect(57, bodyY + 56, pixelSize * 3, Math.max(1, Math.floor(pixelSize / 2)))
            ctx.fillRect(105, bodyY + 56, pixelSize * 3, Math.max(1, Math.floor(pixelSize / 2)))
            break

        case 'sneakers':
            ctx.fillStyle = color
            // Chaussures de sport (pieds avant)
            ctx.fillRect(63, bodyY + 54, pixelSize * 3, pixelSize)
            ctx.fillRect(60, bodyY + 60, pixelSize * 4, pixelSize)
            ctx.fillRect(93, bodyY + 54, pixelSize * 3, pixelSize)
            ctx.fillRect(90, bodyY + 60, pixelSize * 4, pixelSize)
            break


        case 'boots':
            ctx.fillStyle = color
            // Bottes (pieds avant et arrière)
            ctx.fillRect(63, bodyY + 48, pixelSize * 2, pixelSize * 2)
            ctx.fillRect(60, bodyY + 54, pixelSize * 4, pixelSize * 2)
            ctx.fillRect(93, bodyY + 48, pixelSize * 2, pixelSize * 2)
            ctx.fillRect(90, bodyY + 54, pixelSize * 4, pixelSize * 2)
            ctx.fillRect(120, bodyY + 48, pixelSize * 2, pixelSize * 2)
            ctx.fillRect(117, bodyY + 54, pixelSize * 4, pixelSize * 2)
            ctx.fillRect(150, bodyY + 48, pixelSize * 2, pixelSize * 2)
            ctx.fillRect(147, bodyY + 54, pixelSize * 4, pixelSize * 2)
            break

        case 'slippers':
            ctx.fillStyle = color
            // Pantoufles (pieds avant seulement)
            ctx.fillRect(60, bodyY + 57, pixelSize * 5, pixelSize * 2)
            ctx.fillRect(90, bodyY + 57, pixelSize * 5, pixelSize * 2)
            break
